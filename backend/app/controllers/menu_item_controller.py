...
from app.status_codes import HTTP_200_OK, HTTP_201_CREATED, HTTP_400_BAD_REQUEST, HTTP_404_NOT_FOUND, HTTP_500_INTERNAL_SERVER_ERROR
import logging
...
def populate_menu_items():
    default_items = [
        {"name": "Orange Juice", "price": 5000, "category": "DRINKS", "description": "Freshly squeezed orange juice", "image_key": "meal1.jpg"},
        {"name": "Pineapple Juice", "price": 5000, "category": "DRINKS", "description": "Fresh pineapple juice", "image_key": "meal2.jpg"},
        {"name": "Tea and Bread", "price": 4000, "category": "DRINKS", "description": "Hot tea with fresh bread", "image_key": "meal3.jpg"},
    ...
    try:
        ...
        db.session.rollback()
        logging.error(f"Error populating menu items: {str(e)}", exc_info=True)
...
def get_all_menu_items():
    try:
        ...
        logging.error(f"Error fetching menu items: {str(e)}", exc_info=True)
...
def get_menu_item(id):
    try:
        ...
        logging.error(f"Error fetching menu item {id}: {str(e)}", exc_info=True)
    ...
    try:
        ...
        return jsonify({
            ...
            "menu_item": {
                ...
                "image_key": new_item.image_key
            }
        }), HTTP_201_CREATED
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error creating menu item: {str(e)}", exc_info=True)
...
def update_menu_item(id):
    try:
        ...
        db.session.rollback()
        logging.error(f"Error updating menu item {id}: {str(e)}", exc_info=True)
...
def delete_menu_item(id):
    try:
        ...
        db.session.rollback()
        logging.error(f"Error deleting menu item {id}: {str(e)}", exc_info=True)
